...
#define ADMIN 62 // Admin ID
#define ADMIN_PASSWORD "imt2022" // Admin Password
...
void checkBookAvailability(int clientSock) {
    ...
    handleReadError(read(clientSock, &bid, sizeof(int)));
    ...
    for(int i = 0; i<MAX_BOOKS; i++){
        ...
        if(book->bid == bid){
            ...
            book->bid, book->tit, book->auth, book->yr, book->copies, buf);
            handleWriteError(write(clientSock, buffer, strlen(buffer)));
            // Book found
            flag = 1;
            break;
...
void userFunctions(int uid, int clientSock) {
    // User functions
    ...
    while(1){
        ...
        if (choice == 0) {
            ...
            for(int i = 0; i<MAX_USERS; i++){
                if(users[i].uid == uid){
                    for(int j = 0; j<MAX_BOOKS; j++){
                        if(users[i].borrowed[j] != 0){
                            for(int k = 0; k<MAX_BOOKS; k++){
                                if(books[k].bid == users[i].borrowed[j]){
                                    char temp[500];
                                    sprintf(temp, "Book ID: %d\tTitle: %s\tAuthor: %s\tYear: %d\n", 
                                    books[k].bid, books[k].tit, books[k].auth, books[k].yr);
                                    strcat(buffer, temp);
                                }
                            }
                        }
            ...
            for(int i = 0; i<MAX_USERS; i++){
                if(users[i].uid == uid){
                    for(int j = 0; j<MAX_BOOKS; j++){
                        if(users[i].borrowed[j] == bid){
                            // Book found
                                for(int k=j; k<MAX_BOOKS-1; k++){
                                    // Shifting the borrowed books
                                users[i].borrowed[k] = users[i].borrowed[k+1];
                            }
        ...
        if (choice == 0) {
            ...
            for(int i = 0; i<MAX_BOOKS; i++){
                if(books[i].bid > 0){
                    ...
                    books[i].bid, books[i].tit, books[i].auth, books[i].yr, books[i].copies);
                    strcat(buffer,temp);
                }
            }
            // Sending the message to the client
            ...
            for(int i = 0; i<MAX_USERS; i++){
                if(users[i].uid > 0){
                    ...
                    users[i].uid, users[i].name, users[i].pwd);
                    strcat(buffer,temp);
                }
            }
            handleWriteError(write(clientSock, buffer, strlen(buffer)));
            ...
            handleReadError(read(clientSock, &user->uid, sizeof(user->uid)));
            handleReadError(read(clientSock, &user->name, sizeof(user->name)));
            handleReadError(read(clientSock, &user->pwd, sizeof(user->pwd)));
            ...
            int uid;
            handleReadError(read(clientSock, &uid, sizeof(int)));
...
void *handleClient(void *arg) {
    ...
    while(1 && clientSock != -1 && entrflg == 0) {
        ...
        if (choice == 1) {
            ...
            if(adminID == ADMIN && strcmp(pwd, ADMIN_PASSWORD) == 0){
            ...
            for(int i = 0; i<MAX_USERS; i++) {
                ...
                if(user->uid == uid && strcmp(user->pwd, pwd) == 0){
